...
public class NfsPermanentStorageTest {
  ...
  private static final String NONEXISTENT_NFS_STORAGE_PATH =
      "src/test/resources/nfs-storage-test-2";
  private static final String NFS_TASK_STAGE_DIR_PATTERN =
      "%s" + File.separator + "%s" + File.separator + "%s" + File.separator + "%s";
  ...
  private static class TestGenerateAccessLink {
    private static class Inputdownloading {
      ...
      private static final String URL =
          String.format(NFS_STORAGE_TASK_URL_PATTERN, MOCK_NFS_BASE_URL, MOCK_TASK_ID)
              + File.separator
  ...
  public void setUp() throws IOException {
    ...
    properties.setProperty(SapsPropertiesConstants.SAPS_TEMP_STORAGE_PATH, MOCK_TEMP_STORAGE_PATH);
    properties.setProperty(
        SapsPropertiesConstants.PERMANENT_STORAGE_TASKS_DIR, MOCK_NFS_TASKS_DIR_NAME);
    properties.setProperty(SapsPropertiesConstants.PERMANENT_STORAGE_BASE_URL, MOCK_NFS_BASE_URL);
  }
  ...
  public void testArchiveOnDebugMode() throws InvalidPropertyException, IOException {
    ...
    properties.setProperty(
        SapsPropertiesConstants.NFS_PERMANENT_STORAGE_PATH, MOCK_NFS_STORAGE_PATH);
    properties.setProperty(
        SapsPropertiesConstants.PERMANENT_STORAGE_DEBUG_TASKS_DIR, MOCK_NFS_DEBUG_TASKS_DIR_NAME);
  ...
  public void testDelete() throws Exception {
    ...
    FileUtils.copyDirectoryToDirectory(
        taskTempDir, new File(MOCK_NFS_STORAGE_PATH + File.separator + MOCK_NFS_TASKS_DIR_NAME));
    File taskDir =
        new File(
            String.format(
                NFS_STORAGE_TASK_DIR_PATTERN,
  ...
  public void testGenerateAccessLinksTaskDir()
    ...
    exceptedAccessLinksList.add(
        new AccessLink(
            TestGenerateAccessLink.Inputdownloading.NAME,
            TestGenerateAccessLink.Inputdownloading.URL));
    exceptedAccessLinksList.add(
        new AccessLink(
            TestGenerateAccessLink.Preprocessing.NAME, TestGenerateAccessLink.Preprocessing.URL));
    exceptedAccessLinksList.add(
        new AccessLink(
            TestGenerateAccessLink.Processing.NAME, TestGenerateAccessLink.Processing.URL));
  ...
  private boolean assertTaskDir(String taskDirName, String taskId) {
    File inputDir =
        new File(
            String.format(
                NFS_TASK_STAGE_DIR_PATTERN,
                MOCK_NFS_STORAGE_PATH,
                taskDirName,
                taskId,
                INPUTDOWNLOADING_DIR));
    File preprocessingDir =
        new File(
            String.format(
                NFS_TASK_STAGE_DIR_PATTERN,
                MOCK_NFS_STORAGE_PATH,
    ...
    File processingDir =
        new File(
            String.format(
                ...
                taskId,
                PROCESSING_DIR));
    return inputDir.exists()
        && preprocessingDir.exists()
        && processingDir.exists()
        && containsFile(inputDir, TestFile.INPUTDOWNLOADING)
        && containsFile(preprocessingDir, TestFile.PREPROCESSING)
