...
public class SwiftPermanentStorage implements PermanentStorage {
	...
	public SwiftPermanentStorage(Properties properties, SwiftAPIClient swiftAPIClient) throws Exception {
		...
		this.debugMode = properties.containsKey(SapsPropertiesConstants.SAPS_DEBUG_MODE)
				&& properties.getProperty(SapsPropertiesConstants.SAPS_DEBUG_MODE).toLowerCase().equals("true");
	...
	private boolean checkProperties(Properties properties) {
		String[] propertiesSet = { SapsPropertiesConstants.PERMANENT_STORAGE_TASKS_DIR,
	...
	public boolean archive(SapsImage task) {
		...
		String swiftExports = (task.getState() == ImageTaskState.FAILED && this.debugMode) ? debugTasksDirName
				: tasksDirName;

		String inputdownloadingLocalDir = String.format(SAPS_TASK_STAGE_DIR_PATTERN, nfsTempStoragePath, taskId,
				INPUTDOWNLOADING_DIR);
		String inputdownloadingSwiftDir = String.format(SWIFT_TASK_STAGE_DIR_PATTERN, swiftExports, taskId,
				INPUTDOWNLOADING_DIR);

		String preprocessingLocalDir = String.format(SAPS_TASK_STAGE_DIR_PATTERN, nfsTempStoragePath, taskId,
				PREPROCESSING_DIR);
		String preprocessingSwiftDir = String.format(SWIFT_TASK_STAGE_DIR_PATTERN, swiftExports, taskId,
				PREPROCESSING_DIR);

		String processingLocalDir = String.format(SAPS_TASK_STAGE_DIR_PATTERN, nfsTempStoragePath, taskId,
				PROCESSING_DIR);
		...
		boolean processingSentSuccess = preprocessingSentSuccess
				&& archive(taskId, processingLocalDir, processingSwiftDir);
	...
	private boolean archive(String taskId, String localDir, String swiftDir) {
		...
		if (!localFileDir.exists() || !localFileDir.isDirectory()) {
			LOGGER.error(
					"Failed to archive task [" + taskId + "]. " + localDir + " folder isn't directory or not exists");
			return false;
		}
	...
	private List<String> listFiles(String taskId) throws IOException, TaskNotFoundException {
		...
		if (!this.swiftAPIClient.existsTask(this.containerName, this.tasksDirName, taskId)) {
			throw new TaskNotFoundException("Task [" + taskId + "] was not found in directory [" + this.tasksDirName
					+ "] of container [" + this.containerName + "]");
		}
	...
	private List<AccessLink> generateLinks(List<String> filesPaths) {
		...
		try {
			for (String filePath : filesPaths) {
				...
				String link = this.objectStoreServiceApiUrlHost + generateTempURL(filePath) + "&filename="
						+ p.getFileName();
	...
	private String generateTempURL(String filePath) throws NoSuchAlgorithmException, InvalidKeyException {

		String path = this.objectStoreServiceApiUrlPath + File.separator + this.containerName + File.separator
				+ filePath;
	...
	private String runHMACAlgorithm(String data, String key) throws NoSuchAlgorithmException, InvalidKeyException {
